...
class PhyController:
    ...
    def __init__(
        ...
        self.uav_flowgraph_control_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM,
    ...
    #         self.sink_selector_flush_url = "{0}block/{1}/call/2/".format(url, sink_selector_id)
    ...
    def select_phy(self, phy):
        ...
        json_data = json.dumps({
            ...
            "freq_offset_source": self.rx_freq_offset[phy],
            "freq_offset_sink": self.tx_freq_offset[phy],
            "gain_rx": self.rx_gain[phy],
            "gain_tx": self.tx_gain[phy],
            "sample_rate": self.sample_rate[phy],
        })
        self.uav_flowgraph_control_socket.sendto(bytes(json_data, encoding="utf-8"),
                                                 (self.url.split(':')[0], int(self.url.split(':')[1])))
        ...
        #         requests.post(self.sink_selector_flush_url, json=None)
