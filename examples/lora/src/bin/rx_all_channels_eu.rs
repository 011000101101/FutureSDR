...
fn main() -> Result<()> {
    ...
    for n_out in 0..NUM_CHANNELS_PADDED {
        ...
        for sf in SpreadingFactor::iter() {
            ...
            let fft_demod = FftDemod::new(SOFT_DECODING, sf.into(), ldro(sf));
            let gray_mapping = GrayMapping::new(SOFT_DECODING);
            let deinterleaver = Deinterleaver::new(SOFT_DECODING, ldro(sf), sf);
            ...
            connect!(fg,
                resampler [Circular::with_size((1 << 12) * 64 * 4)] frame_sync > fft_demod > gray_mapping > deinterleaver > hamming_dec > header_decoder;
