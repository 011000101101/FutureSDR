 ...
 */
#[inline]
pub fn build_upchirp(id: usize, sf: usize, os_factor: usize, preamble: bool) -> Vec<Complex32> {
    let n = 1 << sf;
    // let preamble = false;
    let id = if preamble {
        id
...
pub fn sample_count(
    ...
    let preamble_symbol_count: f32 =
        preamble_len as f32 + 4.25 + if sf < SF7 && !LEGACY_SF_5_6 { 2.0 } else { 0.0 };
    ...
    ((preamble_symbol_count
        ...
        + ((payload_symbol_count_before_interleaving + header_symbol_count_before_interleaving
            - (Into::<usize>::into(sf) - if LEGACY_SF_5_6 || sf >= SF7 {2} else {0})) as f32
